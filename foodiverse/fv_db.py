...
def create_impact_table(meta, engine):

    with engine.connect() as conn:
        q='DROP TABLE IF EXISTS impact;'
        qr=conn.execute(q)

    donation_windows = Table('impact', meta,
    Column('id', BigInteger, primary_key=True, autoincrement=True),
    Column('name', String),
    Column('organisation_name', String),
    Column('official_id', String),
    Column('countries_id', String),
    Column('provinces_id', String),
    Column('counties_id', String),
    Column('branch_name', String),
    Column('total', Float),
    Column('total_weight', Float),
    Column('total_co2e', Float),
    Column('total_meals', Float),
    Column('in_line_address', String),
    Column('tags', JSON),
    Column('country', JSON),
    Column('province', JSON),
    Column('county', JSON),
...
def delete_window(window_uuid, conn, table):
    ...
    if slct.rowcount!=0:
        ...
        res=None
        err=f'row not found for window_uuid={window_uuid}'
...
def get_cws(donor_uuid_list, conn):
    ...
    q=f"select cw.id, cw.uuid, cw.nm_uuid , m.branch_name, cw.starts_at , cw.ends_at , cw.day_of_week_start , \
        cw.primary_branch_uuid , cw.primary_branch_name, cw.primary_branch_official_id , cw.primary_starts_at_date , cw.primary_scheduled_activation_date , cw.primary_tsm_current_state , cw.primary_uuid, cw.max_collectors,\
        cw.invited_uuid, cw.invited_branch_uuid , cw.invited_branch_name, cw.invited_branch_official_id, cw.invited_starts_at_date, cw.invited_tsm_current_state , cw.invited_scheduled_activation_date ,\
        cw.waitlists ,\
...
def get_uploader_windows_azure(uuids_list, page_size=75, retries=5):
    url=f'https://fv-webhooks.azurewebsites.net/api/get_donor_schedules?code={get_schedules_fn_key}'
    
    windows={}
    grp_size=page_size
    ids_sublists = [uuids_list[x:x+grp_size] for x in range(0, len(uuids_list), grp_size)]
    print(f'Total Groups: {len(ids_sublists)}')
    for i,sublist in enumerate(ids_sublists):
        print(f'\nStarting group {i}')
        retry = 0
        while retry < retries:
            body={"donor_uuids":sublist}
            # uuids_list=json.dumps(valid_records.uuid.to_list())
            body=json.dumps(body)
            # print(body)
            windows_chunk = requests.request('GET', url, data=body)
            if windows_chunk.status_code==200:
                windows_chunk = json.loads(windows_chunk.content)
                windows.update(windows_chunk)
                ...
                if retry < 4:
                    ...
                    print(f'\n\n\nError: {windows_chunk.status_code} : {windows_chunk.content}')
                    print('Error with Function App output for get_windows(). Please try again later or raise with OrgEx if the issue persists')
                    print('\n\n\n')
                continue
...
get_schedules_fn_key = 'HrdRGZSTG06v/vjK0KKtxjsM3Q5kuPGdYQ8KDua8zHG/pyvh1IQeaQ=='
get_members_fn_key = 'zgrSaVLXQsLA0ghuvDereMPraaPmW3l4NUlogIJmaRTa2an2yIgZ6w=='
refresh_members_fn_key = 'lJh3TS8azIaZ6SdPF3K76fW8tpPdaWXSN6xVfeXq5luk/UJS6D9KQQ=='


if __name__ == '__main__':

    # engine=create_engine('postgresql+psycopg2://read_only:password@fv-rep-v1.postgres.database.azure.com:5432/postgres?sslmode=require', echo=False) 
    # meta=MetaData()
    # conn=engine.connect()
